...
public class Answer {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test_case = 1; test_case <= T; test_case++) {
			...
			int M = Integer.parseInt(split[0]); // 총 이동시간
			...
			for (int turn = 0; turn < M + 1; turn++) {
				...
				for (int i = 0; i < A; i++) {
					...
					if (distA <= bcs[i].c) {
						visitedA[i] = 1; // 해당 BC 내부에 방문했다고 표시
					}
				...
				// max는 한칸 이동했을 때 최대로 얻을 수 있는 성능 값
				// answer에 누적해서 답을 구한다.
				answer += max;

				// (디버그용)
				// sb.append("turn: " + turn + " / " + max + " / " + answer + "\n");
			}
